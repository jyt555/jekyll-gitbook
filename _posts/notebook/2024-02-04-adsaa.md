...
为了解决“P是否为NP真子集”的问题，我们尝试解决NPC问题（这里的解决指找到多项式复杂度的解法），如果明确不能解决，则P是NP的真子集。

`NP-Complete Problems`– hardest：等价的，只要解决一个，所有问题都能解决

An NP-complete problem has the property that any problem in NP can be **polynomially** reduced to it.

如果一个问题A能够通过多项式时间复杂度转化成另一个问题B，则A和B是一个等价类（均为P问题，或均为NP问题）

...
Graph G = (V, E) with integer edge weights w (positive or negative). If **we < 0**, where e = (u, v), then u and v want to have the **same state**; if **we > 0** then u and v want **different states.** The absolute value |we| indicates the strength of this requirement.
...
| =>肯定会稳定，因为好边的权重和最大为所有权重和，而每次调整，好边权重都会至少增加1，因此最多需要$W=\sum_e|w_e|$次翻转便可稳定。 |
...
在候选人随机到达的情况下，$E|X|=\sum_{i=1}^N1/i=lnN+O(1),\;cost=O(C_hlnN+NC_i)$
